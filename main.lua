...
print("Setting particle limits to: " .. maxTextureSize)

gWorld = World(love.graphics.getDimensions())
gBoids = Boids(gWorld, maxTextureSize)
gBalls = Dynamic(gWorld, maxTextureSize)
gGame = {
  pause = false
}
...
function love.update(dt)
  ...
  if love.mouse.isDown(2) then
    for i=1,5 do  -- Reduced spawn rate for better control with huge limits
